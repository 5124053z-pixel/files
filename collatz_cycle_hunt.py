...
「もっと大きい数の方がループしそう」か:
  期待ループ数は (候補数 10^d) x (1個が戻る確率 ~ 1/n_0) ~ O(1) となり、
  桁数によらずほぼ一定になる。つまり「大きい方が有望」とは言えないが、
  「大きい方にもあってよい」は正しい（実際 k=190537 なら n_0 は約9万桁）。
  これはコラッツのループについてよく使われる発見的議論で、
  「あるとすればどの大きさでもおかしくない」ことを意味する。
  期待値が同じである以上、選ぶべきは手数を最も稼げる k であり、
  それが k=306（上の TARGETS のコメント参照）。
...
# 探索対象の (k, S)。S は ceil(k*log2(3)) 以上でなければ 2^S > 3^k を
# 満たさず、n_0 が正にならない。
#
# なぜ k=306 だけを回すのか（実測にもとづく）:
#   期待ループ数 ~ (候補数 10^d) x (1個が戻る確率 ~ 1/n_0 ~ 10^-d) = O(1)
#   となり、どの k を選んでも「ループが存在する期待値」はほぼ同じ。
#   一方、探索スループットは n_0 の桁数に強く依存する（実測）:
#
#     k       n_0桁数   個/秒      1年でカバーできる割合
#     306        149    289,710    10^-136
#     665        317    164,691    10^-304
#     15601     7448      4,661    10^-7437
#     31867    15205      1,758    10^-15194
#
#   平均脱落ステップはどの k でも 3.5 で一定なので、差は k ではなく
#   巨大整数演算のコストから来る。期待値が同じなら手数を稼げる方が有利で、
#   k=306 が最良。それでも1年で 10^-136 にしかならないが、
#   他の k はさらに桁違いに悪い。
TARGETS = [
    (306, 485),
]
